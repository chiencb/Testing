...
soap_url=`grep aiafbusiness.soap.address.location $CONFIG_FILE | cut -d = -f 2`
soap_url="$soap_url?wsdl"
soap_url=${soap_url//\//\\/} # replace "/" to "\/"
soap_url=${soap_url//\./\\.} # replace "." to "\."
soap_package_name=`grep aiafbusiness.generated.package $CONFIG_FILE | cut -d = -f 2`
...
soap_url=`grep case360.soap.address.location $CONFIG_FILE | cut -d = -f 2`
soap_url="$soap_url?wsdl"
soap_url=${soap_url//\//\\/} # replace "/" to "\/"
soap_url=${soap_url//\./\\.} # replace "." to "\."
soap_package_name=`grep case360.generated.package $CONFIG_FILE | cut -d = -f 2`
soap_package_name=${soap_package_name//\./\\.} # replace "." to "\."
plugin_replace_statement=${TEMPLATE_ADD_PLUGIN/WSDL_URL/$soap_url}
plugin_replace_statement=${plugin_replace_statement/PACKAGE_NAME/$soap_package_name}
`sed -i $plugin_replace_statement pom.xml`
